...
Deno.test({
  ...
  fn: async (t) => {
    ...
    await t.step('returns 400 when bidding counterpicks not enabled', async () => {
      const leagueId = await factory.createActiveLeague(uniqueName('counterpick-no-bidding'))
      const serviceClient = getServiceClient()

      // Ensure bidding_counterpick_slots is 0
      await serviceClient
        .from('leagues')
        .update({ bidding_counterpick_slots: 0 })
        .eq('id', leagueId)

      // Get a movie from opponent
      const { data: userData } = await client.auth.getUser()
      const { data: participant } = await serviceClient
        .from('league_participants')
        .select('id, teams(id)')
      ...
      if (opponentPicks && opponentPicks.length > 0) {
        ...
        assertEquals(result.error, 'Bidding counterpicks are not enabled for this league')
      }
    })

    await t.step('allows counterpick in active phase when bidding slots enabled', async () => {
      const leagueId = await factory.createActiveLeague(uniqueName('counterpick-bidding'))
      const serviceClient = getServiceClient()

      // Enable bidding counterpicks
      await serviceClient
        .from('leagues')
      ...
      if (opponentPicks && opponentPicks.length > 0) {
        ...
        assertEquals(error, null)
        assertExists(data.counterpick)
        assertEquals(data.counterpick.phase, 'bidding')
      }
    })

    await t.step('returns 400 when bidding counterpick slots are exhausted', async () => {
      const leagueId = await factory.createActiveLeague(uniqueName('counterpick-exhausted'))
      const serviceClient = getServiceClient()

      // Enable bidding counterpicks with only 1 slot
      await serviceClient
        .from('leagues')
      ...
      // Get opponent movies (not user 1's)
      const { data: opponentPicks } = await serviceClient
        .from('draft_picks')
        .select('movie_id')
        .eq('league_id', leagueId)
      ...
      if (opponentPicks && opponentPicks.length >= 2) {
        // Make first counterpick (uses up the slot)
        const result1 = await client.functions.invoke('make-counterpick', {
          body: {
            league_id: leagueId,
            movie_id: opponentPicks[0].movie_id,
          },
        ...
        // Try to make a second counterpick (should fail)
        const result2 = await invokeFunction(client, 'make-counterpick', {
          league_id: leagueId,
          movie_id: opponentPicks[1].movie_id,
        })
        assertEquals(result2.error, 'You have used all your bidding counterpick slots')
      }
    })
