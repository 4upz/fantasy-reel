...
- A **released** movie is droppable (`20260825120000_rescore_team_on_drop.sql`), matching Fantasy Critic. Counterpicks are the only thing that locks a movie to a roster. Two consequences follow, and both are load-bearing:
  - A drop can now remove points that are already banked, so the `dropped_at` transition on `draft_picks`/`pickups` fires `rescore_team_on_holding_drop()`. Don't rescore from the calling Edge Function instead — `drop-movie`, `process-bids` cashing a conditional drop, and admin scripts all set `dropped_at`, and the trigger is the one place that covers them all. `recalculate_teams_for_movie()` cannot: it fans out from a *score* change, and a drop moves ownership while the score stands still.
  - `place-bid` refuses a released movie, so a released drop cannot be picked up by anyone — `drop-movie` therefore skips the `pickup_available` notification for it (gated on the shared `isUpcomingMovie()`, so a movie with no release date is also not announced) and returns `available_for_pickup: false`.
...
- **Counterpicks are tradeable assets in their own right** (`20260822120000_allow_trading_counterpicks.sql`). A trade item's `source` is `'draft_pick' | 'pickup' | 'counterpick'`; for a counterpick, `source_id` is `counterpicks.id`. Ownership is `counterpicker_team_id` — there is no `dropped_at` to check, since a counterpick survives a drop of its movie.
- The invariant is that **no team may finish a trade holding both a movie and the counterpick against it** (`counterpicks_not_own_movie`). That rejects two shapes: a counterpicked movie sent to its counterpicker, and a counterpick sent to the team holding its target. Both are judged on **post-trade** ownership, so the swap — movie one way, the counterpick on it the other — is legal and does execute.
- Because both of a counterpick's team columns can move in one trade and the CHECK is not deferred, `execute_trade()` moves every roster holding first and then calls `settle_counterpicks_for_trade()`, which writes `counterpicker_team_id` and `target_team_id` in a **single statement**. There is no valid one-column-at-a-time ordering — don't reintroduce one. That function supersedes `retarget_counterpicks_for_holding()`, which was dropped.
- `execute_trade()` also syncs the denormalized `draft_picks`/`pickups.counterpicked_by_team_id` and rescores **both** teams (counterpick points move between them).
- Counterpick slot capacity (`draft_counterpick_slots` / `bidding_counterpick_slots`, counted per phase) is enforced in `_shared/trade-validation.ts`, alongside `validateRosterSpace` — counterpicks do not consume roster slots.
