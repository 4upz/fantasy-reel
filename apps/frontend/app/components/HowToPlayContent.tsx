...
export default function HowToPlayContent({ publicView = false }: { publicView?: boolean }): React.ReactElement {
  return (
        ...
        <div className="space-y-16">
          ...
          <div className="card p-8 text-center border-gold/30 bg-gradient-to-br from-surface to-gold-muted/10">
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              <Link href={publicView ? '/signup' : '/dashboard'} className="btn btn-primary">
                {publicView ? 'Start a league' : 'Go to dashboard'}
              </Link>
              <Link href={publicView ? '/login' : '/movies'} className="btn btn-secondary">
                {publicView ? 'Sign in' : 'Browse movies'}
              </Link>
            </div>
          </div>
        </div>
